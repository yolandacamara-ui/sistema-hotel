...
Edad
 @endsection
 @section('contenido')
        <form action="{{ action([App\Http\Controllers\EdadController::class, 'save']) }}" method="POST">
             {{csrf_field()}}  <!--sirve para proteger los formularios contra ataques, es un token -->
            <input type="hidden" value="{{$edad->id}}" name="id">
            
             <div class="form-group">
                <label for="exampleInputEmail1">Edades</label>
                <input type="text" class="form-control" value="{{$edad->nombre}}" name="edad">
            </div>
