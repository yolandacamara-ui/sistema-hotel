...
class MonstruoController extends Controller
{
    function index()
    {
        ...
        $lista = Monstruo::all();
        $datos['lista'] = $lista;
        return view('Monstruo.index')->with($datos); //si el index estuviese en una carpeta distinta, seria Jugdor.index (NombredelaCarpeta.index)
    }
    ...
    function formulario($id = 0)
    {
        ...
        if ($id != 0) {
            ...
            //TipoJugador::find($id);
            $datos['monstruo'] = Monstruo::find($id);
            $datos['operacion'] = 'Editar';
        } else {
            $datos['monstruo'] = new Monstruo();
    ...
    { //public porque el metodo puede ser llamado desde cualquier parte del codigo,
        ...
        $path = storage_path('app/private/monstruos/' . $archivo); //ruta de la foto lo concatena con la variable $archivo que usamos anteriormente
       ...
       // dd($archivo);
        ...
        switch ($context['operacion']) {
            case 'Agregar':
                ...
                $t = new Monstruo(); //Monstruo.php (modelo)
                ...
                if ($peticion->hasFile('foto')) { //aqui pregunta si hay un archivo foto en el campo foto
                    ...
                    $archivo_subido = $archivo->storeAS('monstruos/', $nombre_archivo); //en esta linea storeAS guarda en la carpeta fotos con el nombre de $nombre_archivo
            ...
            case 'Editar':
                $t = Monstruo::find($context['id']); //busca el registro por la llave primaria
                $t->nombre = $context['nombre']; //nombre y puntos se toman de $context porque alli se guardaron los datos
                $t->idnivel = $context['nivel'];
                $t->save(); //guarda los cambios
                ...
                if ($peticion->hasFile('foto')) { //aqui pregunta si se subi una foto en el campo foto, si se subio entonces sigue con el proceso de la condicion, si no, entonces se salta ese paso
                    ...
                    if ($t->foto != '') { //aqui pregunta si hay una foto en el campo foto, si hay una foto en ese campo, se continua la condicion, sino, se salta
                        Storage::delete('monstruos/' . $t->foto);
                    }
                    //2.-se sube
            ...
            case 'Eliminar':
                $t = Monstruo::find($context['id']);
                if ($t->foto != '') { //si la foto es diferente a la cadena vacia
                    //borro el archivo del servidor
                    Storage::delete('monstruos/' . $t->foto); //borra la foto
                }
                $t->delete();
                break;
        }

        return redirect()->route('index_monstruo'); //le esta diciendo cada operacion que haga (editar, agregar o eliminar) lo va a regresar a la ruta
    }
}
