...
class ServicioKPI
{
    ...
    function total_ventas($objeto){
        ...
        if(!isset($objeto->idhabitacion)){
            $objeto->idhabitacion=0;
        }
        ...
        if($objeto->idhabitacion!=0){
                $consulta->where('habitacion.id',$objeto->idhabitacion);
            }
    ...
    function total_ventas_producto($objeto){
        ...
        /*
        SELECT edad.nombre, COUNT(*) as total
        FROM cliente
        JOIN edad on cliente.idedad = edad.id
        GROUP by edad.id;
         */
        function demograficos_edades($objeto){
        ...
        /*
        select habitacion.nombre, cliente.genero, sum(reservacion.precio*reservacion.num_noches) as total, DATE_FORMAT(orden.fecha, '%m-%Y') as fecha
        from orden
        JOIN reservacion on reservacion.idorden = orden.id
        JOIN habitacion on reservacion.idhabitacion = habitacion.id
        JOIN categoria_habitacion on habitacion.tipo = categoria_habitacion.id
        ...
        where orden.fecha>=DATE_SUB(now(), INTERVAL 3 MONTH)
        GROUP by habitacion.id, habitacion.nombre, cliente.genero, DATE_FORMAT(orden.fecha, '%m-%Y')
        ORDER by sum(reservacion.precio*reservacion.num_noches) desc;
         */
        function total_ventas_productoxgenero($objeto){
        if(!isset($objeto->genero))
